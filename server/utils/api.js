const axios = require('axios').default;
 require("dotenv").config();
  ...
  const vaidateTwitch = async (res, req) => {
   try {
    const response = await axios.get(`https://id.twitch.tv/oauth2/token?client_id=${process.env.TWITCH_CLIENT_ID}&client_secret=${process.env.TWITCH_CLIENT_SECRET}&grant_type=client_credentials`);
